...
#' @keywords internal
#' @export parini
parini <- function(averages, funname_df, x, guess, lapses,
                   pariniset, parini) {

  one_parini <- function(averages, funname_df, x, guess, lapses) {
    if (funname_df$funname %in% names(get_functions())) {
      ...
      if (funname_df$funname == 'logistic_fun') p2 <- 1 / p2
      if (funname_df$funname  == 'weibull_fun') p2 <- 1 / p2
  ...
  if (!pariniset) {
    # parini <- apply_to_two_elements(averages, funname_df,
    #                                 ~one_parini(.x, .y, x, guess, lapses))
    parini <- apply_fun(list(averages, funname_df))
